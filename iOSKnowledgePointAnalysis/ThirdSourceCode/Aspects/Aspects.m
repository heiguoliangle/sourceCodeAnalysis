...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    ...
    Method targetMethod = class_getInstanceMethod(klass, selector); // 获取对应类对象/ 元类对象的method
    IMP targetMethodIMP = method_getImplementation(targetMethod); // 通过method 拿到imp
    if (!aspect_isMsgForwardIMP(targetMethodIMP)) {// imp不是_objc_msgForward
        // Make a method alias for the existing method implementation, it not already copied.
        const char *typeEncoding = method_getTypeEncoding(targetMethod);
        SEL aliasSelector = aspect_aliasForSelector(selector); // 拼接自己的selector
        if (![klass instancesRespondToSelector:aliasSelector]) { // 如果不能响应,就添加(原来没有实现)
